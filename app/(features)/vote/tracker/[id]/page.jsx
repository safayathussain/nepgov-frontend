...
const calculateResults = (options) => {
  ...
  return options.map((item) => ({
    ...item,
    percentage: totalVotes > 0 ? ((item.votedCount / totalVotes) * 100).toFixed(0) : 0,
  }));
};
...
const VoteTrackerPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const tracker = trackersResponse.data.data.find((item) => item._id === id);
        if (!tracker) {
  ...
  return (
    <div>
      ...
      <div className="bg-white p-8 rounded-lg mt-5" role="main" aria-label="Voting tracker">
        ...
        {!result.length ? (
          <>
            <div className="lg:m-3 space-y-5 pt-5" role="radiogroup" aria-label="Voting options">
          ...
          <div className="lg:m-3 space-y-5 pt-5 w-full" role="region" aria-label="Voting results">
            {result.map((item) => (
              <div key={item._id} className="flex items-center">
                ...
                <div className="w-full">
                  <div className="flex justify-between w-full">
                    <p>{item.content}</p>
                    <p aria-label={`${item.percentage}% of votes`}>{item.percentage}%</p>
                  </div>
                  <div>
                    <ProgressBar 
